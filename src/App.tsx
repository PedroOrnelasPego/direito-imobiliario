...
const AdBanner = ({ slot, label = "Publicidade", debug = false }: { slot?: string; label?: string; debug?: boolean }) => {
  useEffect(() => {
    if (slot) {
      try {
        // @ts-ignore
  ...
  return (
    <div className="container mx-auto px-6 py-4 flex justify-center">
      <div className={`w-full max-w-4xl min-h-[50px] flex flex-col items-center justify-center relative ${debug ? 'bg-slate-50 border border-dashed border-slate-200 rounded-xl' : ''}`}>
        {debug && (
          <>
            <div className="absolute top-2 right-2 text-[8px] text-slate-400 font-bold uppercase tracking-widest">{label}</div>
            <div className="text-slate-300 text-[10px] uppercase font-bold tracking-tighter flex items-center gap-2">
               <span className="w-6 h-6 rounded-full bg-white flex items-center justify-center border border-slate-100 italic font-serif">Ad</span>
               Espaço reservado para Slot: {slot || "Nenhum"}
            </div>
          </>
        )}
